...
# Fetch a mangafire /api/* endpoint FROM INSIDE the loaded page, signing the
# request with the site's own protection token. MangaFire added a `vrf` token
# (generated by getProtectionToken) that its API now requires; the cleanest way
# to satisfy it is to let the page's own JS produce it. Retries transient
# 5xx/rate-limit responses with backoff.
# MangaFire signs each /api/* request with a `vrf` token generated in-page by
# getProtectionToken(fullPath, queryParamsObject) — the 2nd arg is the query
# params as a typed object (page/limit as numbers). We build the request the
# same way and let the page generate the token, then fetch same-origin.
_API_GET_JS = r"""
async ({path, params, tries}) => {
  const mkTok = (p, q) => { try { return getProtectionToken(p, q); } catch (e) { return null; } };
  let last = 'unknown';
  for (let i = 0; i < tries; i++) {
    const vrf = mkTok(path, params);
    if (!vrf) { last = 'no-token (getProtectionToken missing/changed?)'; break; }
    const sep = path.includes('?') ? '&' : '?';
    try {
      const r = await fetch(path + sep + 'vrf=' + encodeURIComponent(vrf),
                            { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
      if (r.status === 200) return { ok: true, data: await r.json() };
      last = 'HTTP ' + r.status;
      if ([429, 500, 502, 503, 504, 521, 522, 523, 524].includes(r.status)) {
        await new Promise(res => setTimeout(res, 1500 * (i + 1)));
        continue;
      }
      return { ok: false, status: r.status, last };   // e.g. 403 — don't hammer
    } catch (e) {
      last = String(e);
      await new Promise(res => setTimeout(res, 1500 * (i + 1)));
    }
...
def api_get(page, path_base, params=None, retries: int = 5):
    """GET a mangafire JSON API through the page, signed with its vrf token.
    `path_base` is the path without query (e.g. /api/chapters/123); `params` is
    an ordered dict of query params whose value types matter (ints for
    page/limit). Returns parsed JSON, or exits with a clear message."""
    params = params or {}
    qs = "&".join(f"{k}={params[k]}" for k in params)
    path = path_base + (f"?{qs}" if qs else "")
    result = page.evaluate(_API_GET_JS, {"path": path, "params": params, "tries": retries})
    if result.get("ok"):
        return result["data"]
    last = result.get("last", "?")
    if any(s in last for s in ("521", "522", "523", "524")):
        sys.exit(f"✗ MangaFire's servers are down right now ({last}) — not a "
                 f"problem with the tool. Try again in a little while.")
    sys.exit(f"✗ API {path} failed ({last}). MangaFire may have changed its "
             f"anti-scraping token (see README); re-run, or try --headed.")


def list_chapters(page, hid: str, lang: str):
    """All chapters for a title as [{'id':…, 'number':…}, …], ascending."""
    items, page_no = [], 1
    while True:
        d = api_get(page, f"/api/titles/{hid}/chapters",
                    {"language": lang, "sort": "number", "order": "asc",
                     "page": page_no, "limit": 100})
        batch = d.get("items", [])
        items.extend(batch)
        if len(batch) < 100:
            return items
...
def download_chapter(page, session, origin: str, chap_id: str, out_dir: Path, workers: int = 8):
    """Download one chapter, fetching its pages in parallel.
    Returns (folder, meta dict, saved names) or None.
    `page` is used only for the token-signed page-list API call; the images
    themselves come down through `session` (plain parallel HTTP, no token)."""
    d = api_get(page, f"/api/chapters/{chap_id}", {})
    try:
        data = d["data"]
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        if args.chapters:
            ...
            all_ch = list_chapters(page, hid, args.lang)
            if not all_ch:
        ...
        for gi, chunk in enumerate(chunks, 1):
            ...
            for cid in chunk:
                n += 1
                print(f"\n[{n}/{len(chap_ids)}]", end=" ")
                res = download_chapter(page, session, origin, cid, out_dir, args.workers)
                if res and res[2]:
